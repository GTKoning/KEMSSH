...
#if defined(WITH_OQS) &&                                                       \
    defined(WITH_PQ_KEX) // This is fine because if OQS is active, we have TQS
                         // active as well.
...
static int input_pq_tqs_reply(int type, u_int32_t seq, struct ssh *ssh);
static int pq_tqs_s2c_deserialise(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx,
                                  struct sshkey **server_host_key,
                                  u_char **server_host_key_blob,
                                  size_t *server_host_key_blob_len);
static int pq_tqs_s2c_deserialisever(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx);
...
static int pq_tqs_c2s_serialise(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx);
static int pq_tqs_c2s_serialise2(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx);
static int pq_tqs_verify_hostkey(struct ssh *ssh,
                                 struct sshkey *server_host_key);
static int pq_tqs_deserialise_hostkey(struct ssh *ssh,
                                      struct sshkey **server_host_key,
                                      u_char **server_host_key_blob,
                                      size_t *server_host_key_blob_len);
...
static int pq_tqs_s2c_deserialise(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx,
   ...
   */
  // They immediately verify host key after getting it, is this realistic? Check
  // schematic (think it is) where do they store the pk_b? in the blob, where do
  // they store the ct_b? in pq_kex_ctx->oqs_kex_ctx->oqs_remote_msg (en length)
  if ((r = pq_tqs_deserialise_hostkey(ssh, server_host_key,
                                      server_host_key_blob,
                                      server_host_key_blob_len)) != 0 ||
      (r = tqs_deserialise2(ssh, pq_kex_ctx->oqs_kex_ctx, TQS_IS_CLIENT)) !=
          0) {
...
static int pq_tqs_s2c_deserialisever(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx) {
  int r = 0;
  if ((r = tqs_deserialisever(ssh, pq_kex_ctx->oqs_kex_ctx, TQS_IS_CLIENT) !=
           0))
    goto out;
  r = sshpkt_get_end(ssh);
out:
...
static int pq_tqs_c2s_serialisever(struct ssh *ssh, PQ_KEX_CTX *pq_kex_ctx) {
 ...
 */
int pq_tqs_client(struct ssh *ssh) {
  ...
  /* Basically sends pk_a, it's in ctx as local msg and local private is sk_a.
   * Do keep track of local msg as this can be overwritten */
  ...
  ssh_dispatch_set(ssh, tqs_ssh2_reply_msg(oqs_alg), &input_pq_tqs_reply);
...
static int input_pq_tqs_reply(int type, u_int32_t seq, struct ssh *ssh) {
  ...
  // pk_b is stored in server_host_key struct (or in blob), ct_b in remote_msg
  // van ctx
  if ((r = pq_tqs_s2c_deserialise(ssh, pq_kex_ctx, &server_host_key,
                                  &server_host_key_blob,
                                  &server_host_key_blob_len)) != 0) {
  ...
  // Getting the shared secret by decapsulating -> not the way we want to do it
  // actually. Wij willen eerst encapsulaten, dus deze functie moet worden
  // aangepast. Sws moet de struct eigenlijk worden meegegeven als argument(denk
  // ik). Probeer met enkel het blob
  if ((r = tqs_client_shared_secret(
           oqs_kex_ctx, &tqs_key_a, &tqs_key_b, &tqs_full_key,
           &tqs_fullkey_size, &tqs_halfkey_size, &server_host_key)) != 0) {
    goto out;
  }
  ...
  // dump_value("remote msg", oqs_kex_ctx->oqs_remote_msg,
  ...
  if ((r = pq_tqs_hash(kex->hash_alg, kex->client_version_string,
                       kex->server_version_string, oqs_kex_ctx->oqs_local_msg,
                       oqs_kex_ctx->oqs_local_msg_len,
                       oqs_kex_ctx->oqs_remote_msg,
                       oqs_kex_ctx->oqs_remote_msg_len, tqs_full_key,
  ...
  */
  ...
  //    if ((r = sshpkt_put_string(ssh, oqs_kex_ctx->tqs_ct_a,
  ...
  // dump_value(" [DISGESTA die wordt gebruikt in tqs_reply] ", digest,
  ...
  // dump_value(" [PRINTING HASH CLIENT]", pq_kex_ctx->oqs_kex_ctx->hash,
  // pq_kex_ctx->oqs_kex_ctx->hash_len);
...
static int pq_tqs_verinit(int type, u_int32_t seq, struct ssh *ssh) {
  ...
  // dump_value(" [DISGESTA die wordt gebruikt] ",
  // ssh->kex->pq_kex_ctx->oqs_kex_ctx->digesta,
  // ssh->kex->pq_kex_ctx->oqs_kex_ctx->digestlen); dump_value(" [PRINTING HASH
  // CLIENT]", pq_kex_ctx->oqs_kex_ctx->hash,
  // pq_kex_ctx->oqs_kex_ctx->hash_len); error(" digesta should be sent NU NOG
  // GOED"); error(" waiting for digestb");

  debug("expecting %i msg verreply", tqs_ssh2_verreply_msg(oqs_alg));
  ssh_dispatch_set(ssh, tqs_ssh2_verreply_msg(oqs_alg), &input_pq_tqs_verreply);
out:
  return r;
}
...
static int input_pq_tqs_verreply(int type, u_int32_t seq, struct ssh *ssh) {
  ...
  // dump_value(" [DISGESTA die wordt gebruikt in verreply] ",
  // oqs_kex_ctx->digesta, oqs_kex_ctx->digestlen); dump_value(" [PRINTING HASH
  // CLIENT in verreply]", pq_kex_ctx->oqs_kex_ctx->hash,
  // pq_kex_ctx->oqs_kex_ctx->hash_len);

  if ((r = pq_tqs_s2c_deserialisever(ssh, pq_kex_ctx)) != 0) {
    goto out;
  }
  // error("Did I get digestB?");
  // dump_value("digestb", pq_kex_ctx->oqs_kex_ctx->digestb,
  // pq_kex_ctx->oqs_kex_ctx->digestlen);
  ...
  // error(" Wats de size g: %zu ", oqs_kex_ctx->digestlen +
  ...
  // oqs_kex_ctx->tqs_fullkey_size); dump_value(" [DIGESTA die wordt gebruikt]
  // ", oqs_kex_ctx->digesta, oqs_kex_ctx->digestlen); dump_value(" [DIGESTB die
  // wordt gebruikt] ", oqs_kex_ctx->digestb, oqs_kex_ctx->digestlen);
  // dump_value(" [MACMESSAGE die wordt gebruikt] ", macmessage,
